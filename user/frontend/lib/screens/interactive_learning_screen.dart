...
class _InteractiveLearningScreenState extends State<InteractiveLearningScreen>
  ...
  IconData _iconFor(String concept) {
    final match = dogConcepts.where((c) => c.concept.toLowerCase() == concept.toLowerCase());
  ...
  Widget build(BuildContext context) {
    final explanation =
        conceptExplanations[_currentConcept] ?? 'Explore attributes and relationships for this concept.';
    final match = dogConcepts.where((c) => c.concept.toLowerCase() == _currentConcept.toLowerCase());
    final conceptValue = match.isNotEmpty ? match.first.value : _currentConcept;
    final color = _colorFor(_currentConcept);
    final attributeImage =
        widget.attributeImages[_attributeKey(_currentConcept)] ??
        widget.attributeImages[_currentConcept.toLowerCase()] ??
        widget.attributeImages[_currentConcept] ?? '';

    // Other concepts for bottom chips (exclude current)
    final otherConcepts =
        dogConcepts.where((c) => c.concept.toLowerCase() != _currentConcept.toLowerCase()).toList();
